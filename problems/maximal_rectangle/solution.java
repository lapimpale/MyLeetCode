class Solution {
   public int maximalRectangle(char[][] matrix) {
        ...
        for (int i = 0; i < r; i++)
            for (int j = 0; j < c; j++)
                if (matrix[i][j] == '1')
                    dp[i][j] = dp[Math.max(0, i - 1)][j] + 1; //populate heights array.

        int max = 0; // the following is from my solution for 84. Largest Rectangle in Histogram
		             // just put it in a for-loop and that's it.
        for (int[] heights : dp){
